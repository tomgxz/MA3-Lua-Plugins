...
function pos_get_groups(DP)
        ...
        if userInput ~= '' and userInput ~= nil then
            ...
            if userInput ~= DONE then
                ...
                elseif type(userInput) ~= 'table' then
                    table.insert(pos_groups,userInput)
                        if #pos_groups > 1 then
                            last_line = #pos_groups
                            local ind = 1
                            local deleted
                            repeat
                                if pos_groups[ind] == pos_groups[last_line] then
                                    table.remove(pos_groups,last_line)
                                    deleted = true
                ...
                elseif type(userInput) == 'table' then
                    for ind = 1, #userInput do
                        local exists
                        for idx = 1, #pos_groups do
                            if pos_groups[idx] == userInput[ind] then
                                exists = true
                            end
                        end
                        if not exists then
                            table.insert(pos_groups,userInput[ind])
                        end
                    end
                end
            else
    ...
    -- for k,v in pairs(pos_groups) do
    --     Echo(k..' '..v)
    -- end
    return pos_groups
end